...
async function proxyMaintenanceState(): Promise<NextResponse> {
  try {
    const response = await fetch(`${resolveApiUpstreamBaseUrl()}/health/maintenance`, {
      cache: "no-store",
      redirect: "manual"
    });
    ...
    if (!response.ok) {
      const message =
        payload && typeof payload === "object" && "message" in payload
          ...
          : "Maintenance endpoint unavailable";
