...
async function proxyReadinessState(): Promise<NextResponse> {
  try {
    const response = await fetch(`${resolveApiUpstreamBaseUrl()}/health/readiness`, {
      cache: "no-store",
      redirect: "manual"
    });
