...
const CONTRACT_HISTORY_PAGE_SIZE_OPTIONS = [10, 20, 50, 100] as const;

export function ContractsHistoryTable({ contracts, isLoading }: ContractsHistoryTableProps) {
  const [search, setSearch] = useState("");
  const [pageSize, setPageSize] =
    useState<(typeof CONTRACT_HISTORY_PAGE_SIZE_OPTIONS)[number]>(20);
      ...
      <CardContent className="space-y-3 pt-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(value) =>
              setPageSize(
                Number.parseInt(value, 10) as (typeof CONTRACT_HISTORY_PAGE_SIZE_OPTIONS)[number]
              )
            }
          >
            ...
            <SelectContent>
              {CONTRACT_HISTORY_PAGE_SIZE_OPTIONS.map((size) => (
